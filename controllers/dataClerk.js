...
router.get("/packageList/delete/:packageID", dataSession, (req, res) => {
  packageData
    .deletePackage(req.params.packageID)
    .then(() => {
      res.redirect("/dataClerk/packageList");
    })
    .catch((err) => {
      res.status(500).send("Unable to Remove Package / Package Not Found");
    });
});
...
router.post("/package/update/:packageID", dataSession, (req, res) => {
  packageData
    .updatePackage(req.params.packageID, req.body)
    .then(() => {
      res.redirect("/dataClerk/PackageList");
    })
    .catch((err) => {
...
router.post("/package/add", upload.single("image"), dataSession, (req, res) => {
  packageData
    ...
    .then(() => {
      packageData
        .updatePackage(req.body.packageID, {
